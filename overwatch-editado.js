    ...
    playerData.forEach(player => {
        if (player.playerVillages) {
            player.playerVillages.forEach(village => {
                ...
                totalPop = 0;
                game_data.units.forEach((element, j) => {
                    currentPop = currentPop + ((village.unitsInVillage[element] || 0) * unitPopValues[game_data.units[j]]);
                    totalPop = totalPop + ((village.unitsInVillage[element] || 0) * unitPopValues[game_data.units[j]]) + (parseInt(village.unitsEnroute[element]) * unitPopValues[game_data.units[j]]);
                });
                let wt;
